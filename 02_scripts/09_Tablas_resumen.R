...
a0_Matriz_inicial %>%
  ...
  # LaTeX
  kbl(caption = "Legalization treatment groups by monitoring group",
...
a0_Matriz_inicial %>%
  ...
  rename(`Tratment group`=1) %>% 
  # LaTeX
  kbl(caption = "MIB treatment groups by monitoring group",
      booktabs = T, align = "c", format = "latex") %>% 
  kable_styling(font_size = 8) %>% 
  add_header_above(c("","Monitoring frequency group" =3), italic = T) %>% 
  collapse_rows(columns = c(1,5), latex_hline = "none") %>% 
  pack_rows("Pre-court intervention",1,2) %>%
  pack_rows("Post-court intervention",3,20, hline_before =  T) %>% 
  pack_rows(" ",21,22, hline_before = T)
